...
function delNodes(
  ...
  const ans: TreeNode[] = [];
  function dfs(node: TreeNode | null, isRoot: boolean) {
    if (!node) return null;
    const shouldDel = set.has(node.val);
    node.left = dfs(node.left, shouldDel);
    node.right = dfs(node.right, shouldDel);
    isRoot && !shouldDel && ans.push(node);
    if (shouldDel) node = null;
    return node;
  }
  dfs(root, true);
  return ans;
